class VoiceCallsController < ApplicationController
  ...
  def index
    ...
    @voice_calls = user.voice_calls(:order => [ :created_at.desc ]).paginate :page => params[:page] #, :per_page => 5
    ...
    if current_user 
      ...
      respond_to do |format|
        if voice_call.save
          ...
          call_url = sys_config.tropo_url + '?action=create&token=' + profile.voice_token + '&to=' + callto + '&from=' + firstnumber + '&ov_action=call&user_id=' + current_user.to_s
